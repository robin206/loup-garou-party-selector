  ...
  <header className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-md border-b border-gray-800">
    <div className="container mx-auto px-4 h-16 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        <Link to="/rules" state={gameState} className="flex items-center gap-1 text-gray-300 hover:text-werewolf-accent">
...
const Header = () => {
  // Try to use location, but provide a fallback if not in a Router context
  let gameState;
  try {
  ...
  return <HeaderContent gameState={gameState} />;
};
